...
describe('transaction reducer', () => {
  ...
  describe('finalizeTransaction', () => {
    ...
    it('sets receipt', () => {
      ...
      const tx = store.getState()[ChainId.LUMIO]?.['0x0'];
      expect(tx?.summary).toEqual('hello world');
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.MAINNET), String(ChainId.LUMIO)]);
      expect(Object.keys(store.getState()[ChainId.MAINNET] ?? {})).toEqual(['0x0']);
      expect(Object.keys(store.getState()[ChainId.LUMIO] ?? {})).toEqual(['0x1']);
